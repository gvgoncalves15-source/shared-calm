...
async function init() {
  const { data, error } = await sb.from('profiles').select('id,name,role,avatar_url,accent,monthly_income').order('id');
  if (error) {
...
async function loadFinancas() {
  ...
  /* Tendência real: gastos deste mês x mês passado */
  const thisMonth = monthRangeStrings(0), lastMonth = monthRangeStrings(-1);
  const sumInRange = (r) => txs.filter(t => t.tx_date >= r.start && t.tx_date <= r.end).reduce((s, t) => s + Number(t.amount), 0);
  const thisMonthSum = sumInRange(thisMonth), lastMonthSum = sumInRange(lastMonth);
  ...
  renderDonutChart(txs.filter(t => t.tx_date >= thisMonth.start && t.tx_date <= thisMonth.end));
  ...
  const renderFiltered = () => {
    ...
    if (!filtered.length) {
      txWrap.innerHTML = txs.length
        ? '<p class="text-on-surface-variant font-body-md text-[14px] text-center py-4">Nenhuma despesa encontrada com esse filtro.</p>'
        : '<p class="text-on-surface-variant font-body-md text-[14px] text-center py-4">💰 Nenhuma despesa registrada ainda.<br/>Adicione a primeira tocando no botão +.</p>';
    }
  };
...
async function renderPodemosGastar(wrap, casalTxs, casalGoals, thisMonth) {
  ...
  const despesasMes = casalTxs.filter(t => t.tx_date >= thisMonth.start && t.tx_date <= thisMonth.end).reduce((s, t) => s + Number(t.amount), 0);
  ...
  const disponivel = income - despesasMes - contasPendentes - metaContrib;
...
function openTxModal(mode, tx) {
  ...
  document.getElementById('tx-modal-title').textContent = mode === 'edit' ? 'Editar Despesa' : 'Nova Despesa';
...
on('form-tx', 'submit', async (e) => {
  ...
  const { error } = txModalMode === 'edit'
    ? await sb.from('finance_transactions').update({ description, amount, category, scope }).eq('id', txModalEditId)
    : await sb.from('finance_transactions').insert({ description, amount, category, scope, owner: state.currentProfile, tx_date: todayLocal() });

  closeModals();
  document.getElementById('form-tx').reset();
  if (error) { toast('Erro ao salvar despesa: ' + error.message, true); return; }
  toast(txModalMode === 'edit' ? 'Despesa atualizada' : 'Despesa adicionada');
  loadFinancas();
});
