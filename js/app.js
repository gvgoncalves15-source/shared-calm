...
/* No novo modelo, state.profiles só contém quem está no MESMO lar que você
   (o RLS do banco já garante isso sozinho). O parceiro é simplesmente "o
   outro perfil do lar", se existir. */
function getPartnerProfile() {
  return state.profiles.find(p => p.id !== state.currentProfile) || null;
}
...
async function refreshProfilesAndPartner() {
  const { data } = await sb.from('profiles').select('id,name,role,avatar_url,accent,monthly_income').order('id');
...
/* ---------------- CONVITE (detecção de ?invite= na URL) ---------------- */

(function detectInviteInUrl() {
  const params = new URLSearchParams(window.location.search);
  const code = params.get('invite');
  if (code) {
    localStorage.setItem('velo_pending_invite', code);
    window.history.replaceState({}, '', window.location.pathname);
  }
  if (localStorage.getItem('velo_pending_invite')) {
...
sb.auth.onAuthStateChange((event) => {
  if (event === 'PASSWORD_RECOVERY') openModal('modal-new-password');
});

/* Garante que existe um perfil (e um lar) para essa conta autenticada —
   cria na primeira vez que a pessoa realmente entra (seja na hora, seja
   depois de confirmar o e-mail). Também aplica um convite pendente, se
   houver, e então entra no app. */
async function enterWithSession(session) {
  const user = session.user;
  let { data: profile } = await sb.from('profiles').select('id,name,role,avatar_url,accent,monthly_income').eq('id', user.id).maybeSingle();

  if (!profile) {
    let pending = null;
    try { pending = JSON.parse(localStorage.getItem('velo_pending_signup') || 'null'); } catch (e) { pending = null; }
    const name = (pending && pending.name) || (user.email ? user.email.split('@')[0] : 'Usuário');
    const role = (pending && pending.role) || '';
    const accent = (pending && pending.accent) || 'gold';

    const { data: hh, error: hhError } = await sb.from('households').insert({}).select().single();
    if (hhError) { toast('Erro ao criar seu lar: ' + hhError.message, true); return; }

    const { data: newProfile, error: pError } = await sb.from('profiles')
      .insert({ id: user.id, household_id: hh.id, name, role, accent, avatar_url: avatarUrl(name, accent) })
      .select().single();
    if (pError) { toast('Erro ao criar perfil: ' + pError.message, true); return; }
    profile = newProfile;
    localStorage.removeItem('velo_pending_signup');

    const pendingInvite = localStorage.getItem('velo_pending_invite');
    if (pendingInvite) {
      const { data: joinResult } = await sb.rpc('join_household_by_invite', { p_code: pendingInvite });
      localStorage.removeItem('velo_pending_invite');
      if (joinResult === 'ok') toast('Contas vinculadas! 🔗');
    }
  }
...
on('toggle-auth-mode-btn', 'click', () => {
  const loginForm = document.getElementById('form-login');
  const signupForm = document.getElementById('form-signup');
  const label = document.getElementById('toggle-auth-mode-label');
  const icon = document.getElementById('toggle-auth-mode-icon');
  ...
  label.textContent = showingSignup ? 'Já tenho conta' : 'Criar nova conta';
  icon.textContent = showingSignup ? 'login' : 'person_add';
});
...
on('form-login', 'submit', async (e) => {
  ...
  if (error) {
    status.textContent = error.message.includes('Invalid login') ? 'E-mail ou senha incorretos.' : 'Erro ao entrar: ' + error.message;
    return;
  }
...
on('forgot-password-btn', 'click', async () => {
  const email = document.getElementById('login-email').value.trim() || await promptValue('Digite seu e-mail para receber o link de redefinição:');
  if (!email) return;
  const { error } = await sb.auth.resetPasswordForEmail(String(email), { redirectTo: window.location.origin + window.location.pathname });
  const status = document.getElementById('login-status');
  status.textContent = error ? 'Erro: ' + error.message : 'Enviamos um link de redefinição para o seu e-mail.';
});
...
on('form-signup', 'submit', async (e) => {
  ...
  if (password !== passwordConfirm) { status.textContent = 'As senhas não coincidem.'; return; }

  localStorage.setItem('velo_pending_signup', JSON.stringify({ name, role, accent: 'gold' }));
  const { data, error } = await sb.auth.signUp({ email, password });
  if (error) { status.textContent = 'Erro ao criar conta: ' + error.message; return; }

  document.getElementById('form-signup').reset();
  if (data.session) {
    await enterWithSession(data.session);
  } else {
    status.textContent = 'Conta criada! Confira seu e-mail para confirmar antes de entrar.';
  }
});

on('form-new-password', 'submit', async (e) => {
  e.preventDefault();
  const p1 = document.getElementById('new-password-1').value;
  const p2 = document.getElementById('new-password-2').value;
  if (p1 !== p2) { toast('As senhas não coincidem', true); return; }
...
function enterApp(userId) {
  ...
  state.rotinaTab = userId;
  state.financasTab = userId;
  state.fixasTab = userId;
  state.perfilMonthTab = userId;
...
function goTo(view) {
  ...
  if (view === 'perfil') { renderLinkPanels(); loadPerfilSummary(); }
}
...
on('switch-profile-btn', 'click', async () => {
...
on('edit-profile-btn', 'click', () => openProfileModal());
...
on('form-profile', 'submit', async (e) => {
  ...
  if (!name) return;
  if (password && password !== passwordConfirm) { toast('As senhas não coincidem', true); return; }
  ...
  await refreshProfilesAndPartner();
  if (state.currentView === 'perfil') renderLinkPanels();

  const updated = profileById(state.currentProfile);
  document.getElementById('header-avatar').src = updated.avatar_url;
  document.getElementById('perfil-avatar').src = updated.avatar_url;
  document.getElementById('perfil-name').textContent = updated.name;
  document.getElementById('perfil-role').textContent = updated.role;
...
let linkTabsWired = false;

function renderLinkPanels() {
  if (!linkTabsWired) {
    linkTabsWired = true;
    on('link-tab-mine', 'click', () => switchLinkTab('mine'));
    on('link-tab-join', 'click', () => switchLinkTab('join'));
    on('form-join-link', 'submit', async (e) => {
      e.preventDefault();
      const raw = document.getElementById('join-link-input').value.trim();
      const status = document.getElementById('join-link-status');
      status.textContent = '';
      if (!raw) return;
      let code = raw;
      try {
      ...
      } catch (_) { /* não é uma URL, trata como código puro */ }

      const { data: result, error } = await sb.rpc('join_household_by_invite', { p_code: code });
      if (error) { status.textContent = 'Erro: ' + error.message; return; }
      if (result === 'not_found') { status.textContent = 'Link ou código inválido.'; return; }
      status.textContent = '';
...
function switchLinkTab(tab) {
  document.getElementById('link-tab-mine').classList.toggle('active', tab === 'mine');
  document.getElementById('link-tab-join').classList.toggle('active', tab === 'join');
  document.getElementById('link-panel-mine').classList.toggle('hidden', tab !== 'mine');
  document.getElementById('link-panel-join').classList.toggle('hidden', tab !== 'join');
}

async function renderMyLinkPanel() {
  const panel = document.getElementById('link-panel-mine');
  const partner = state.partner;
    ...
    panel.innerHTML = `
      <div class="flex items-center gap-3">
        ...
        <div class="flex-1">
          ...
          <div class="font-label-sm text-label-sm text-gold">🔗 Vinculado</div>
        </div>
      </div>`;
    return;
  }

  panel.innerHTML = '<p class="font-body-md text-[14px] text-on-surface-variant text-center">Carregando seu link...</p>';
  const { data: hh, error } = await sb.from('households').select('invite_code').single();
  if (error || !hh) { panel.innerHTML = '<p class="font-body-md text-[14px] text-error text-center">Erro ao buscar seu link.</p>'; return; }

  const link = `${window.location.origin}${window.location.pathname}?invite=${hh.invite_code}`;
  panel.innerHTML = `
    <p class="font-body-md text-[14px] text-on-surface-variant mb-3">Envie esse link para a pessoa com quem você quer vincular sua conta.</p>
    <div class="flex items-center gap-2 bg-surface-container-low rounded-lg p-sm mb-3">
      <span class="font-body-md text-[13px] truncate flex-1">${link}</span>
    </div>
    <div class="flex gap-2">
      <button id="copy-link-btn" class="shine-btn flex-1 rounded-full bg-primary text-on-primary p-sm font-body-md text-[13px] font-semibold">Copiar link</button>
      <button id="regen-link-btn" class="rounded-full border border-outline-variant p-sm px-3 text-on-surface-variant"><span class="material-symbols-outlined text-[18px]">refresh</span></button>
    </div>`;

  document.getElementById('copy-link-btn').addEventListener('click', async () => {
    try { await navigator.clipboard.writeText(link); toast('Link copiado!'); }
    catch (e) { toast('Não foi possível copiar — selecione o link manualmente', true); }
  });
  document.getElementById('regen-link-btn').addEventListener('click', async () => {
    if (!(await confirmDialog('Gerar um novo link? O link antigo deixará de funcionar.', 'Gerar novo'))) return;
    const { error: regenErr } = await sb.rpc('regenerate_invite_code');
    if (regenErr) { toast('Erro: ' + regenErr.message, true); return; }
...
function closeModals() {
  ...
  ['modal-task', 'modal-tx', 'modal-nudge', 'modal-profile', 'modal-goal', 'modal-fixed', 'modal-confirm', 'modal-value', 'modal-new-password'].forEach(id => document.getElementById(id).classList.add('hidden'));
  const pw = document.getElementById('profile-password');
  if (pw) pw.value = '';
  const pwConfirm = document.getElementById('profile-password-confirm');
...
function refreshCurrentView() {
  ...
  if (state.currentView === 'perfil') { renderLinkPanels(); loadPerfilSummary(); }
}
