...
try {
  ...
  console.error('[SharedCalm] falha ao iniciar Supabase:', err);
...
function on(id, ev, fn) {
  ...
  if (!el) { console.warn('[SharedCalm] elemento não encontrado:', id); return; }
...
function installmentCard(i, fe, status) {
  const div = document.createElement('div');
  const borderColor = status === 'atrasada' ? 'border-error' : status === 'paga' ? 'border-secondary' : 'border-surface-container-low';
...
init().catch(err => {
  console.error('[SharedCalm] erro ao iniciar app:', err);
