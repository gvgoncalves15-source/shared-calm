...
async function loadCasal() {
  ...
  const c1 = countFor(p1.id), c2 = countFor(p2.id);
  const sum = (c1 + c2) || 1;
  ...
  document.getElementById('casal-count-mariana').textContent = `${p2.name} (${c2})`;
...
document.getElementById('send-nudge-btn').addEventListener('click', () => openModal('modal-nudge'));
...
function closeModals() {
  ...
  ['modal-task', 'modal-tx', 'modal-nudge', 'modal-profile'].forEach(id => document.getElementById(id).classList.add('hidden'));
}
